...
class BookingRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function JobType($job_type)
    {
        $translator_type = '';
        if ($job_type == 'paid')
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $$translator_type = $this->JobType($job_type);
    ...
    public function alerts()
    {
        ...
        if ($cuser->is('superadmin')) {
            $allJobs = DB::table('jobs')
                ->join('languages', 'jobs.from_language_id', '=', 'languages.id')->whereIn('jobs.id', $jobId);


            $allJobs->when($requestdata['lang'] != '', function ($query) use ($requestdata) {
                $query->whereIn('jobs.from_language_id', $requestdata['lang'])
                    ->where('jobs.ignore', 0);
            });

            $allJobs->when($requestdata['status'] != '', function ($query) use ($requestdata) {
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        $condition  = array('jobs.status' => 'pending', 'jobs.ignore_expired' => 0, 'jobs.due' => Carbon::now());
